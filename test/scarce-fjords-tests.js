...
module.exports = function(ok) {
  ...
  [5,6,7,8,9].forEach(function(index) {
    var map=maps[index], game=new ENGINE.Game(map,{seed:6}), expected=index===5 ? 9 : index===9 ? 24 : 21;
    var name=map.name, factories=Object.values(game.buildings).filter(function(b){return b.kind==="factory";});
    ok(game.width === (index===5 ? 34 : index===6 ? 30 : index===9 ? 42 : 31) && game.height===(index===5 ? 34 : index===9 ? 20 : 30),
      name+": expected map dimensions");
    ...
    [0,1].forEach(function(player) {
      ...
      var roster=index===5 ? ["CHARLIE","PANTHER","RABBIT"] : index===9 ? ["CHARLIE","OCTOPUS","PANTHER","RABBIT","SLAGGER","TITAN"] : ["BISON","CHARLIE","HADRIAN","PANTHER","POLAR","RABBIT"];
      ok(JSON.stringify(army.map(function(u){return u.t;}).sort())===JSON.stringify(roster),
        name+": player "+player+" has the authored starting army");
      ok(army.every(function(u){return HEX.distance(u.x,u.y,base.col,base.row)===1;}),
        name+": all starters are beside their home camp");
    });
    ...
    factories.forEach(function(b) {
      ...
      if((index===7 || index===8) && b.col===15 && directions.length===2) {
        ok(JSON.stringify(directions)==="[2,5]", name+": the shared two-exit factory has symmetric north/south mouths");
      } else {
        ok(directions.some(function(start){return directions.every(function(d){return (d-start+6)%6<directions.length;});}),
          name+": terminal factory mouths face into one fjord approach");
      }
      ...
      b.stored.forEach(function(u,i) {
        u.player=0;
        ok((u.type.cls!=="infantry" && !u.type.capture) ||
          (index>=8 && ["CHARLIE","KILROY"].includes(u.typeId)), name+": factory reserves use only the permitted infantry");
    ...
    ok(open.length/(game.width*game.height)<(index>=8 ? .7 : .45), name+": floor area leaves room for the mountain walls");
    ...
    var isolated=index===8 ? 8 : index===9 ? 20 : 0;
    ok(reachable(function(p){return map.grid[p.row][p.col]!=="M";}).size===open.length-isolated,
      name+": all fjords connect apart from the deliberately isolated clearings");
    ok(reachable(function(p){return !["M","F"].includes(map.grid[p.row][p.col]);}).size===open.length-factories.length-isolated,
    ...
    ok(restored.units.length===(index===5 ? 6 : 12) && Object.values(restored.buildings).filter(function(b){return b.kind==="factory";}).length===expected,
