...
if(!isMainThread){
  ...
  try{
    if(args.resume){run=JSON.parse(fs.readFileSync(statePath,"utf8"));if(run.sourceHash!==sourceHash)throw new Error("Code changed since this tournament began. Start a new run to keep ratings comparable.");}
      ...
      const input=args.config?JSON.parse(fs.readFileSync(String(args.config),"utf8")):{
        ...
        maxRounds:Number(args.rounds||0),workers:Number(args.workers||Math.min(4,Math.max(1,os.availableParallelism()-1))),work:String(args.work||"standard"),opening:String(args.opening||"original"),noDeal:String(args["no-deal"]||"skip"),seed:Number(args.seed||42),k:Number(args.k||24),selfPlay:!!args["self-play"]};
  ...
  function commit(result){
    ...
    T.rate(run.ratings,result,run.config.k);run.completed++;run.elapsed+=result.ms||0;if(result.error)run.errors++;if(result.skipped)run.skipped=(run.skipped||0)+1;save();
  ...
  function finish(){run.status="complete";save();pool.forEach(s=>s.worker.terminate());console.log(JSON.stringify({completed:run.completed,errors:run.errors,skipped:run.skipped||0,ratings:run.ratings,sourceHash,archive:out},null,2));if(run.errors)process.exitCode=1;}
  function drain(){
