...
var AI_SEARCH = (function () {
  ...
  var modes = [
    ...
    {id:"apex",label:"Apex · hybrid search",description:"Beam-guided Monte Carlo search, deeper replies and wider tactical coverage",algorithm:"hybrid",width:5,depth:3,branches:14,iterations:80,horizon:14}
  ];
  ...
  function shortlist(game,ctx,limit,unitLimit) {
    return model.candidates(game,ctx,{limit:limit,perUnit:3,unitLimit:unitLimit||0});
  }
  ...
  function* beam(game,root,ctx,config,rng) {
    ...
    var nodes=initial.map(function(r){
      var state=model.simulate(game,r.action,ctx,randomSeed(rng));
    ...
    for(var level=1;level<depth;level++){
      ...
      for(var n=0;n<nodes.length;n++){
        ...
        for(var a=0;a<actions.length;a++){
          var action=actions[a], child=model.simulate(node.game,action,ctx,randomSeed(rng));
  ...
  function* treeSearch(game,root,ctx,config,rng,beamValues) {
    ...
    var scored=rootChoices.map(function(c){
      ...
      var estimate=beamValues?(c.total+c.prior*4)/(c.visits+4):mean;
  ...
  function* decide(game,id,options,shared) {
    ...
    return yield* treeSearch(rootGame,roots,ctx,config,rng,beamValues);
  }
  ...
  function publicSnapshot(game) {
    ...
    return {version:1,map:game.map,types:types,units:Object.values(units),field:game.units.map(function(u){return u.id;}),
      buildings:buildings,turn:game.turn,currentPlayer:game.currentPlayer,turnLimit:game.turnLimit,
  ...
  function createTurn(game,player,options) {
    ...
    if(async&&typeof Worker!=="undefined"){
      try{
        worker=new Worker("js/ai-worker.js?v=20260923-search-1");
